...
async function main() {
  ...
  if (!kasRw) kasRw = await prisma.pocket.create({ data: { communityId: community.id, name: 'Kas RW', type: 'treasury' } });
  if (!arisanPocket) arisanPocket = await prisma.pocket.create({ data: { communityId: community.id, name: 'Arisan Bulanan', type: 'arisan' } });
