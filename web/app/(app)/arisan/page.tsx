                ...
                {isAdmin && (
                    <div className="flex items-center gap-3">
                        ...
                        <button onClick={() => setShowPayoutModal(true)}
                            className="flex items-center gap-1.5 px-4 py-2 bg-primary text-white rounded-xl text-xs font-bold hover:brightness-90 transition shadow-sm cursor-pointer">
                            Lakukan Undian
                        </button>
                    </div>
                )}
                                ...
                                {periods.filter(p => p.status === 'done').map((p, i) => (
                                    <tr key={p.id} className="hover:bg-gray-50/30">
                                        <td className="px-6 py-3.5">
                                            <span className="text-xs font-bold text-slate-800">Putaran {i + 1}</span>
                                        </td>
                                        <td className="px-6 py-3.5">
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
                    <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-6">
                        ...
                        <div className="flex gap-3">
                            <button onClick={() => setShowPayoutModal(false)} className="flex-1 py-2 border border-gray-200 rounded-xl text-xs font-bold text-slate-500 hover:bg-slate-50 transition cursor-pointer">Batal</button>
                            <button onClick={handleExecutePayout} className="flex-1 py-2 bg-primary text-white rounded-xl text-xs font-bold hover:brightness-90 transition cursor-pointer">Undi & Cairkan</button>
                        </div>
                    </div>
                </div>
            )}
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
                    <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-2xl space-y-5 max-h-[85vh] flex flex-col">
                        ...
                        <form onSubmit={handleAddMember} className="flex gap-2">
                            ...
                            <button type="submit" className="px-4 py-2 bg-primary text-white text-xs font-bold rounded-xl hover:brightness-90 transition cursor-pointer">Tambah</button>
                        </form>
                        <div className="flex-1 overflow-y-auto space-y-2">
                            {participants.map(m => (
                                <div key={m.id} className="flex items-center justify-between px-4 py-2.5 bg-slate-50 rounded-xl border border-gray-100">
                                    <span className="text-xs font-bold text-slate-800">{m.member?.name}</span>
                                    <span className="text-[9px] font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-100">Aktif</span>
                                </div>
                            ))}
                        </div>
